...
CKEDITOR.dialog.add( 'fuelanchor', function( editor ) {
	...
	return {
		...
		onShow: function() {
			var sel = editor.getSelection(),
				...
				linkElement = fakeSelected ?
					CKEDITOR.plugins.fuelanchor.tryRestoreFakeAnchor( editor, fullySelected ) :
					CKEDITOR.plugins.fuelanchor.getSelectedLink( editor );
		...
		contents: [ {
			id: 'info',
			label: editor.lang.fuelanchor.anchor.title,
			accessKey: 'I',
			elements: [ {
				type: 'text',
				id: 'txtName',
				label: editor.lang.fuelanchor.anchor.name,
				required: true,
				validate: function() {
					if ( !this.getValue() ) {
						alert( editor.lang.fuelanchor.anchor.errorName ); // jshint ignore:line
						return false;
					}
					return true;
				}
